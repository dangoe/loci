...
use crate::memory::store::{MemoryQuery, MemoryQueryMode, MemoryStore};
use crate::memory::{MemoryEntry, Score};
...
impl ContextualizerConfig {
    /// Creates a new `ContextualizerConfig` with the provided values.
    pub fn new(
        text_generation_model: impl Into<String>,
        system: Option<ContextualizerSystemConfig>,
        memory_mode: ContextualizationMemoryMode,
        max_memory_entries: NonZeroUsize,
        min_score: Score,
        ...
        Self {
            text_generation_model: text_generation_model.into(),
            system,
            memory_mode,
            max_memory_entries,
            min_score,
    ...
    /// Returns the text generation model identifier.
    pub fn text_generation_model(&self) -> &str {
        &self.text_generation_model
    }

    /// Returns the optional system config.
    pub fn system(&self) -> Option<&ContextualizerSystemConfig> {
        self.system.as_ref()
    }

    /// Returns the memory mode.
    pub fn memory_mode(&self) -> ContextualizationMemoryMode {
        self.memory_mode
    }

    /// Returns the maximum number of memory entries to retrieve.
    pub fn max_memory_entries(&self) -> NonZeroUsize {
    ...
    /// Returns the optional tuning config.
    pub fn tuning(&self) -> Option<&ContextualizerTuningConfig> {
        self.tuning.as_ref()
    }
}
...
pub struct ContextualizerSystemConfig {
    ...
    mode: ContextualizerSystemMode,
    /// The system prompt template text to use when constructing the system prompt.
    system: String,
}

impl ContextualizerSystemConfig {
    /// Creates a new `ContextualizerSystemConfig`.
    pub fn new(mode: ContextualizerSystemMode, system: impl Into<String>) -> Self {
        Self {
            mode,
            system: system.into(),
        }
    ...
    pub fn mode(&self) -> ContextualizerSystemMode {
        self.mode
    }

    /// Returns the system prompt template text.
...
pub struct ContextualizerTuningConfig {
    ...
    thinking: Option<text_generation::ThinkingMode>,
    stop: Option<Vec<String>>,
    keep_alive: Option<Duration>,
    extra_params: ModelProviderParams,
}

impl ContextualizerTuningConfig {
    /// Creates a new `ContextualizerTuningConfig` with all fields set explicitly.
    ...
    /// Returns the nucleus sampling probability cutoff.
    pub fn top_p(&self) -> Option<f32> {
        self.top_p
    }

    /// Returns the repeat penalty.
    pub fn repeat_penalty(&self) -> Option<f32> {
        self.repeat_penalty
    }

    /// Returns the repeat last n tokens window.
    pub fn repeat_last_n(&self) -> Option<u32> {
        self.repeat_last_n
    }

    /// Returns the thinking mode.
    pub fn thinking(&self) -> Option<&text_generation::ThinkingMode> {
    ...
    /// Returns the keep-alive duration.
...
impl ContextualizationDebugInfo {
    /// Returns the memory entries that were retrieved and injected into the prompt.
    ...
    E: text_generation::TextGenerationModelProvider + 'static,
{
    ...
    async fn contextualize_internal(
        &self,
        prompt: &str,
    ) -> Result<(Vec<MemoryEntry>, ResultStream), ContextualizerError> {
    ...
    fn build_request(&self, prompt: &str, memory_entries: &[MemoryEntry]) -> TextGenerationRequest {
        let mut req =
    ...
    fn build_system_prompt(&self, memory_entries: &[MemoryEntry]) -> String {
    ...
    fn format_memory_block(memory_entries: &[MemoryEntry]) -> String {
    ...
    async fn query_memory(&self, prompt: &str) -> Result<Vec<MemoryEntry>, ContextualizerError> {
        let query = MemoryQuery::new(prompt.into(), MemoryQueryMode::Use)
            .with_max_results(self.config.max_memory_entries)
...
mod tests {
    ...
    use crate::{
        memory::store::{MemoryQuery, MemoryQueryMode},
        memory::{MemoryEntry, Score},
        model_provider::{
    ...
    fn default_config() -> ContextualizerConfig {
        ContextualizerConfig {
            ...
            max_memory_entries: NonZeroUsize::new(5).unwrap(),
    ...
    fn make_entry(content: &str) -> MemoryEntry {
        MemoryEntry::new(content.to_string(), HashMap::new())
    }
    ...
    async fn test_contextualize_applies_tuning_from_config() {
        ...
        assert_eq!(req.stop(), Some(["<END>".to_string()].as_slice()));
    ...
    fn test_query_memory_uses_use_mode() {
        let query = MemoryQuery::new("test".to_string(), MemoryQueryMode::Use)
    ...
    async fn test_contextualize_with_debug_returns_debug_info_and_stream() {
        ...
        assert_eq!(
            debug_info.memory_entries[0].content(),
            "user prefers dark mode"
        );
