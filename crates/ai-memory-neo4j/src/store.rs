...
impl<E: TextEmbedder> Neo4jMemoryStore<E> {
    ...
    pub async fn initialize(&self) -> Result<(), MemoryStoreError> {
        ...
        self.graph
            .run_on(&self.config.database, neo4rs::query(&cypher))
            .await
    ...
    async fn do_save(
        ...
        let mut result = self
            .graph
            .execute_on(&self.config.database, q)
            .await
            .map_err(|e| MemoryStoreError::Query(e.to_string()))?;
...
impl<E: TextEmbedder> MemoryStore for Neo4jMemoryStore<E> {
    async fn save(&self, input: MemoryInput) -> Result<MemoryEntry, MemoryStoreError> {
        ...
        if let Some(threshold) = self.config.similarity_threshold {
            let candidates = self
                .query_by_embedding(&embedding, 1, threshold, &memory.metadata)
                .await?;
    ...
    async fn update(&self, id: Uuid, input: MemoryInput) -> Result<MemoryEntry, MemoryStoreError> {
        ...
        let q = neo4rs::query(&format!(
            "MATCH (m:{NODE_LABEL} {{id: $id}}) \
             SET m.content = $content, m.embedding = $embedding, m.metadata = $metadata \
